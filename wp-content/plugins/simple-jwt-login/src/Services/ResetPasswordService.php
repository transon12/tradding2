...
class ResetPasswordService extends BaseService implements ServiceInterface
{
    ...
    private function validateChangePassword()
    {
        if (empty($this->request['phone'])) {
            throw new Exception(
                __('Missing phone number parameter.', 'simple-jwt-login'),
    ...
    private function sendResetPassword()
    {
        ...
        $pass=$this->request['new_password'];
        $user = $this->wordPressData->getUserByUserLogin($phone);
        if (empty($user)) {
            throw new Exception(
        ...
        switch ($this->jwtSettings->getResetPasswordSettings()->getFlowType()) {
            case ResetPasswordSettings::FLOW_JUST_SAVE_IN_DB:
                $this->wordPressData->generateAndGetPasswordResetKey($phone);
    ...
    private function validateSendResetPassword()
    {
        if (empty($this->request['phone'])) {
            throw new Exception(
                __('Missing phone parameter.', 'simple-jwt-login'),
    ...
    private function getUser($jwtAllowed)
    {
        if ($jwtAllowed && empty($this->request['code'])) {
            ...
            if (empty($user) || $this->wordPressData->getUserProperty($user, 'user_login') !== $this->request['phone']) {
                throw new Exception(
        ...
        $user = $this->wordPressData->checkPasswordResetKey($code, $this->request['phone']);
        if (empty($user)) {
            throw new Exception(
