...
async function getInfoCommon(notShowExtra) {
  ...
  var dataTable = [
    ...
    {
      color: '',
      value: formatnumber(resultPrice?.totalQty*10),
      alias: 'Khối Lượng',
  ...
  var html = '';
  var number1 = resultPrice?.best1OfferPrice - resultPrice?.basicPrice;
  var number2 = (number1 / resultPrice?.basicPrice) * 100;
  html +=
    ' <div class="d-flex py-1"><b class="pl-1 text-dark"> Giá hiện tại: ' +(formatnumber(resultPrice?.best1OfferPrice ?? 0) )+'</b>';
    html += '<div style="flex: 1 1 auto" class="text-right"><b class="text-right" id="label_" style=" color: ' +
    (number1 > 0 ? COLOR_GREEN: COLOR_RED) +
    '" title="">' +(number1 > 0 ? '+' : '') + formatnumber(number1) + ' / </b>' +
    '<b class="text-right" id="label_" style=" color: ' + (number1 > 0 ? COLOR_GREEN: COLOR_RED) + '" title="">' + 
    (number1 > 0 ? '+' : '') + formatnumber(number2) + '%</b></div>';
  html += '</div>';
  if (infoExtra) {
    infoExtra.map((e, index) => {
      ...
      if (e.color) {
        ...
        html += '<div class="py-1"><b>' + e.label + ' :</b>';
        html +=
...
function checkDisplayColor(price = 0) {
  return checkColor(
    price,
    RESULT_PRICE?.ceilingPrice ?? 0,
    RESULT_PRICE?.floorPrice ?? 0,
...
async function statistical() {
  ...
  if (result.length > 0) {
    var dataTable = [
      {
        color: COLOR_YELLOW,
        value: formatnumber(result[0]?.priceBasic),
        alias: 'Tham chiếu',
        isShow: true,
      },
      {
        color: COLOR_BLUE_ACCESS,
        value: formatnumber(result[0]?.priceOpen),
        alias: 'Mở cửa',
        isShow: true,
      },
      {
        ...
        value:
          ...
          COLOR_RED +
          '">' +
          formatnumber(result[0]?.priceLow) +
          '</span> - <span style="color:' +
          COLOR_GREEN +
          '">' +
          formatnumber(result[0]?.priceHigh) +
          '</span></div>',
        alias: 'Thấp - cao',
        isShow: true,
      },
      {
        color: '',
        value: formatnumber(result[0]?.putthroughVolume ?? 0),
        alias: 'KL thỏa thuận',
        isShow: true,
      },
      {
        color: '',
        value: formatnumber(result[0]?.dealVolume ?? 0),
        alias: 'KL khớp',
        isShow: true,
      },
      ...
      {
        color: '',
        value: formatnumber(result[0]?.totalVolume ?? 0),
        alias: 'Tổng KL',
        isShow: true,
      },
      {
        color: '',
        value: formatnumber(
          result[0]?.totalValue ? result[0]?.totalValue / 1000000 : 0
        ),
        alias: 'Tổng giá trị',
        isShow: true,
      },
    ];
    var number1 = result[0]?.priceAverage - result[0]?.priceBasic;
    var number2 = (number1 / result[0]?.priceBasic) * 100;
    
    html2 +=
    ' <div class="d-flex py-1"><b class="pl-1 text-dark"> Giá hiện tại: ' +(formatnumber(result[0]?.priceAverage ?? 0) )+'</b>';
    html2 += '<div style="flex: 1 1 auto" class="text-right"><b class="text-right" id="label_" style=" color: ' +
    (number1 > 0 ? COLOR_GREEN: COLOR_RED) +
    '" title="">' +(number1 > 0 ? '+' : '') + formatnumber(number1) + ' / </b>' +
    '<b class="text-right" id="label_" style=" color: ' + (number1 > 0 ? COLOR_GREEN: COLOR_RED) + '" title="">' + 
    (number1 > 0 ? '+' : '') + formatnumber(number2) + '%</b></div>';
  html2 += '</div>';
    ...
    dataTable.map((e) => {
      ...
      html2 += e.isShow
        ? '<tr>' +
          '<td style="width:65%"> <div class="pl-1">' +
          e.alias +
          '</td>' +
          th() +
          '</tr>'
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (i === 0) {
      htmlVolume +=
        '<tr>' +
        "<td class='text-center'> <b style='color:" +
        COLOR_GREEN +
        "'>" +
  ...
  jQuery('.nav-home-content-2').html(html2);
...
jQuery(document).ready(async function () {
  $select = await jQuery('.normalize').selectize({
    ...
    onDropdownOpen: function () {
      jQuery('.custom-listStock').removeClass('custom-width')
      jQuery('.custom-listStock').addClass('fullWidth');
      jQuery('.selectize-dropdown').addClass('w-100');
    },
    valueField: 'id',
    labelField: 'id',
    searchField: 'id',
    onDropdownClose: function () {
      jQuery('.custom-listStock').removeClass('fullWidth');
      jQuery('.custom-listStock').addClass('custom-width');
    },
    ...
    onChange: async function (e) {
      ...
      await chartPrice();
      checkIndexStock(e);
      selectize2.setValue(selectize2.search(codeStock).items[0].id);
    },
    ...
    render: {
      option: function (item) {
        return (
          "<div class='d-flex' > <div class='p-2 ' style='flex: 1 1 auto'>" +
          item.id +
          "</div><div class='p-2 '> " +
          item.title +
          '</div></div>'
  ...
  $select_2 = await jQuery('.normalize-2').selectize({
    ...
    onChange: async function (e) {
      ...
      await chartPrice();
      checkIndexStock(e);
      selectize.setValue(selectize.search(codeStock).items[0].id);
    },
    onDelete: async function () {
      //  await setData('a')
    },
...
async function setData(text, defaults) {
  ...
  let listCost = await callApi(
    'https://restv2.fireant.vn/search?keywords=' +
      text +
      '&type=symbol&offset=0&limit=200',
    'GET',
    false,
    false,
    false,
